...
GOOGLE_PROFILE_URL = 'https://www.google.com/maps/search/?api=1&query=Terramorph+LLC+Perrysburg+OH'
...
OFFICIAL_NAP_NOTE = 'Terramorph LLC | Perrysburg, OH 43551 | 419-873-6801 | terramorphllc.com'
...
<footer class="footer">
  <div class="container footer-grid">
    ...
    <div>
      ...
      <a href="service-areas.html#official-business-info">Business details</a>
...
<script src="app.js?v=3.63"></script>
'''
...
SERVICES = [
    ...
    ('Outdoor Lighting', 'Light My Property', 'Low-voltage lighting for paths, patios, entries, landscape features, safety, security, and nighttime curb appeal.', ['Path lights', 'Accent lighting', 'Patio lighting', 'Entry lighting', 'Security visibility'], 'assets/real-lighting-night.webp', 'outdoor-lighting.html'),
    ('Lawn Maintenance', 'Set Up Maintenance', 'Residential and commercial mowing, edging, trimming, bed upkeep, brush control, and recurring property presentation.', ['Mowing', 'Edging', 'Trimming', 'Weeding', 'Bed maintenance', 'Commercial maintenance'], 'assets/real-mowing-service.webp', 'lawn-maintenance.html'),
    ('Seasonal Cleanups', 'Schedule Cleanup', 'Spring and fall cleanups, pruning, leaf removal, debris removal, bed resets, overgrowth control, and property preparation.', ['Spring cleanup', 'Fall cleanup', 'Leaf removal', 'Spring pruning', 'Tree and bush trimming', 'Debris hauling'], 'assets/real-fall-leaf-crew.webp', 'seasonal-cleanups.html'),
    ('Snow Removal', 'Get Winter Service', 'Dependable winter service for homes, businesses, and properties that need safe access when Northwest Ohio weather hits.', ['Driveways', 'Walkways', 'Commercial lots', 'Ice management', 'Winter response'], 'assets/snow-removal-truck.webp?v=3.17', 'snow-removal.html'),
]
SERVICE_GROUPS = [
...
PROJECT_PHOTOS = [
    ('Estate lawn and stone-wall property care', 'assets/real-estate-stone-wall.webp'),
    ('Fall leaf cleanup crew at work', 'assets/real-fall-leaf-crew.webp'),
    ('Finished fall cleanup on a wooded property', 'assets/real-fall-cleanup-after.webp'),
    ('Boxwood and mulch foundation beds', 'assets/real-boxwood-front-bed.webp'),
    ('Brick paver walkway and garage beds', 'assets/real-brick-paver-walkway.webp'),
    ('Retaining wall installation', 'assets/real-retaining-block.webp'),
    ('Front-entry columns and fresh plantings', 'assets/real-entry-columns-bed.webp'),
    ('New-construction landscape installation', 'assets/completed-new-construction-landscape.jpg'),
    ('Landscape lighting at night', 'assets/real-lighting-night.webp'),
    ('Deck-side patio landing with white rock', 'assets/real-deck-patio-landing.webp'),
    ('Estate front beds with white rock', 'assets/real-estate-front-bed.webp'),
    ('Lawn maintenance and clean striping', 'assets/real-striped-lawn-pool.webp'),
    ('Backyard garden bed transformation', 'assets/real-backyard-garden-bed.webp'),
    ('Arborvitae privacy row planting', 'assets/real-arborvitae-row.webp'),
    ('Shoreline rock and erosion control', 'assets/real-shoreline-riprap.webp'),
    ('Modern stone wall and plantings', 'assets/real-modern-stone-wall.webp'),
    ('Fence-line planting bed with flowers', 'assets/real-fence-line-planting.webp'),
    ('Mature front-yard bed refresh', 'assets/real-mature-front-bed.webp'),
    ('Tree ring with stone border', 'assets/real-tree-ring-pergola.webp'),
    ('Hydroseed lawn installation', 'assets/real-hydroseed.webp'),
    ('Commercial grounds with rock beds', 'assets/real-commercial-rock-beds.webp'),
    ('Driveway lighting after dark', 'assets/real-lighting-driveway.webp'),
]
...
def head(title, desc, schema='', page_name=''):
    ...
    stylesheet_version = '3.62'
    schema_block = f'\n  <script type="application/ld+json">{schema}</script>' if schema else ''
    if page_name in ('thank-you.html', 'review-notes.html'):
        robots_block = '\n  <meta name="robots" content="noindex, nofollow">'
    elif page_name.startswith('lp-') or page_name == 'case-studies.html' or page_name.endswith('-case-study.html'):
...
def optimize_markup(html):
    ...
    def img_attrs(match):
        ...
        return tag
    html = re.sub(r'<img\b[^>]*>', img_attrs, html)

    def prioritize_hero(match):
        prefix, tag = match.groups()
        tag = re.sub(r'\sloading="[^"]+"', '', tag)
        tag = re.sub(r'\sfetchpriority="[^"]+"', '', tag)
        return prefix + tag[:-1].rstrip() + ' loading="eager" fetchpriority="high">'

    return re.sub(
        r'(<div class="(?:hero-media|page-hero-image)"[^>]*>\s*)(<img\b[^>]*>)',
        prioritize_hero,
        html,
        count=1,
        flags=re.DOTALL,
...
def inline_jobber_quote_form(title='Request My Outdoor Transformation Quote', service='Outdoor Transformation'):
    return f'''
<section class="quote-panel jobber-quote-panel quote-request-section" id="request-form" aria-labelledby="home-request-title" data-service="{service}">
...
<div class="quote-popup" id="quote-popup" role="dialog" aria-modal="true" aria-labelledby="quote-popup-title" aria-describedby="quote-popup-desc" hidden>
  ...
  <section class="quote-popup-card quote-popup-simple" tabindex="-1">
    ...
    <div class="popup-direct-actions">
      ...
      <p>Call now or jump straight to the quote request form.</p>
...
<section class="trust-band" aria-label="Terramorph credibility">
  <div class="container trust-grid">
    ...
    <div class="trust-feature"><strong>Direct</strong><span>Free estimate requests</span><small>Call or send project details through the secure request form.</small></div>
  </div>
</section>'''
...
<section class="conversion-trust-strip priority-fit-strip" aria-label="Terramorph project fit">
  <div class="container conversion-trust-grid">
    <span>Quick-turn service work and full installations welcome</span>
...
def photo_gallery(photos=None):
    slides = ''.join(f'<figure class="carousel-slide"><img src="{img}" alt="Terramorph {label}" loading="lazy"></figure>' for label, img in (photos or PROJECT_PHOTOS))
...
<section class="section clay-section">
  <div class="container local-grid">
    <div>
      ...
      <h2>Locally owned and operated for Northwest Ohio property problems.</h2>
      <p>Terramorph brings practical field experience and a property-first approach to drainage, grading, plant health, hardscape base issues, maintenance needs, and seasonal work before recommending a scope.</p>
    ...
    <div class="authority-list">
      ...
      <div><b>Local accountability</b><span>A Perrysburg-area team serving residential, commercial, and industrial properties across Wood and Lucas County.</span></div>
      <div><b>Practical experience</b><span>Project photos, customer reviews, and field experience give property owners evidence before requesting an estimate.</span></div>
...
<section class="section official-business-section" id="official-business-info" aria-labelledby="official-business-title">
  <div class="container official-business-card">
    <div>
      <p class="eyebrow">Local company</p>
      <h2 id="official-business-title">One Northwest Ohio team for the work around your property.</h2>
      <p>Terramorph is based in the Perrysburg area and serves homeowners, businesses, and property owners across Wood and Lucas County. Call or send the project details to confirm service fit and the next estimate step.</p>
    </div>
    <dl class="nap-grid">
      <div><dt>Company</dt><dd>{BUSINESS_NAME}</dd></div>
      <div><dt>Call</dt><dd><a href="tel:{TEL}">{PHONE}</a></dd></div>
      <div><dt>Online</dt><dd><a href="{BASE_URL}/">terramorphllc.com</a></dd></div>
      <div><dt>Home base</dt><dd>{BUSINESS_CITY}, {BUSINESS_REGION} {BUSINESS_POSTAL}</dd></div>
      <div><dt>Service area</dt><dd>Wood County, Lucas County, Perrysburg, Toledo, Maumee, and Northwest Ohio</dd></div>
      <div><dt>Specialties</dt><dd>{BUSINESS_CATEGORY}</dd></div>
    </dl>
    <p class="fine-print">Free estimates are available for landscaping, patios, drainage, lighting, maintenance, cleanups, snow service, and related outdoor property work.</p>
  </div>
</section>'''
...
<section class="section signature-work-home" aria-label="Terramorph featured outdoor work">
  <div class="container signature-work-layout">
    ...
    <div class="signature-gallery">
      <a class="signature-card large" href="projects.html">
        <img src="assets/real-estate-stone-wall.webp" alt="Terramorph estate lawn and stone-wall property work" loading="lazy">
        <span><b>Estate lawn and property care</b><small>Clean mowing patterns, established landscape edges, and coordinated outdoor presentation.</small></span>
      </a>
      <a class="signature-card" href="completed-new-construction-landscape-project.html">
        <img src="assets/completed-new-construction-landscape.jpg" alt="Terramorph new-construction landscape installation" loading="lazy">
      ...
      <a class="signature-card" href="completed-lawn-maintenance-project.html">
        <img src="assets/real-striped-lawn-pool.webp" alt="Terramorph completed large-lot lawn maintenance" loading="lazy">
        <span><b>Large-lot lawn maintenance</b><small>Consistent striping, broad coverage, and clean presentation around property edges.</small></span>
      </a>
    </div>
  </div>
</section>"""
...
<section class="hero-v2">
  <div class="hero-media"><img src="assets/real-hero.webp" alt="Terramorph landscaping, hardscape, and outdoor property transformation in Northwest Ohio" loading="eager" fetchpriority="high" decoding="async"></div>
  <div class="hero-overlay"></div>
  <div class="container hero-content">
    <div class="hero-copy">
      <p class="eyebrow light">Terramorph LLC | Perrysburg, Ohio</p>
      <h1>Landscape design, patios, and drainage built for Northwest Ohio.</h1>
...
<section class="section quote-section">
  <div class="container quote-grid">
    ...
    <div class="cta-proof">
      <p class="eyebrow">Send the useful details</p>
      <h2>Help the team qualify the project before the first call.</h2>
      <ul class="check-list">
        <li>Property city and address</li>
        <li>Service needed and the problem to solve</li>
        <li>Preferred timeline and access notes</li>
        <li>Current photos of the project area</li>
      </ul>
      <a class="btn btn-outline-light" href="tel:{TEL}">Call {PHONE}</a>
    </div>
  </div>
...
SERVICE_FAQS = {
    'Snow Removal': [
        ('Do you offer seasonal snow removal contracts?', 'Yes. Terramorph offers seasonal winter service for driveways, walkways, and commercial lots in Wood and Lucas County. Late summer and fall are the best time to lock in a spot before routes fill.'),
        ('Do you plow residential driveways or just commercial lots?', 'Both. Terramorph handles residential driveways and walkways as well as commercial parking lots and property access.'),
        ('When do you come out after it snows?', 'Service is triggered by snowfall depth agreed on in the contract. Routes run so driveways and lots are open when people need to leave for work or open for business.'),
        ('Do you handle ice and salting too?', 'Yes. Ice management and salting can be included so walkways, entries, and lots stay safe, not just cleared.'),
        ('What areas do you cover for snow removal?', 'Perrysburg, Toledo, Maumee, Sylvania, Rossford, and surrounding Wood and Lucas County communities.'),
    ],
    ...
    'Seasonal Cleanups': [
        ('What is included in a Terramorph fall cleanup?', 'Leaves cleared from the lawn, beds, fence lines, and under shrubs; spent perennials cut back and annuals pulled; light shrub shaping; sticks, brush, and debris hauled off; bed edges cleaned up; and final mows when they are part of the plan. The leaves and debris leave with the crew.'),
        ('When should I schedule a fall cleanup in Northwest Ohio?', 'Most leaves come down from late October through November in the Toledo area, so that is when the main cleanup visit happens. Book in September or early October. Heavily wooded properties often need one more pass after the last leaves fall.'),
        ('Do you haul the leaves away?', 'Yes. Leaves and debris are hauled off, so there is no waiting on a city curbside leaf program or keeping piles out of the street.'),
        ('Is it bad to leave leaves on the lawn over winter?', 'A thick layer blocks light and can leave thin, matted patches and snow mold in spring. Ohio State University Extension recommends removing leaves once they build up past about two inches.'),
        ('Do you cut back every plant in the beds?', 'No. Spent and diseased perennials get cut back and annuals come out. Spring-flowering shrubs like lilac, forsythia, and azalea are not sheared in fall because next spring\'s flower buds are already set. Ornamental grasses and seed heads can stay up for winter interest if you want.'),
        ('Do you do spring cleanups too?', 'Yes. Spring cleanups clear winter debris, cut back anything left standing over winter, clean and edge the beds, and get the property ready for mowing and mulch.'),
        ('Can a cleanup be paired with aeration, overseeding, or snow removal?', 'Yes. Many properties book fall aeration and overseeding, the cleanup, and a seasonal snow removal contract together so the property is covered from fall into winter.'),
    ] + DEFAULT_FAQS[:2],
}
...
<section class="page-hero premium-page-hero">
  ...
  <div class="container page-hero-content">
    ...
    <h1>{headline}</h1>
    <p>{lead}</p>
    <div class="cta-row"><a class="btn btn-gold" href="{hero_cta_href}">{cta}</a><a class="btn btn-outline-light" href="tel:{TEL}">Call {PHONE}</a></div>
  </div>
</section>
{trust_band()}
{priority_fit_strip(title)}{inline_form}
...
def meta_landing_page(filename, title, eyebrow, image, headline, lead, bullets, service, form_title, offer, symptoms, next_steps, cta_label='Get My Free Assessment'):
...
CLEANUP_PHOTOS = [
    ('Fall leaf cleanup crew at work', 'assets/real-fall-leaf-crew.webp'),
    ('Commercial walk-behind leaf blower on a finished lawn', 'assets/real-fall-windstorm-blower.webp'),
    ('Finished fall cleanup on a wooded property', 'assets/real-fall-cleanup-after.webp'),
    ('Clean striped lawn after a fall cleanup', 'assets/real-fall-finished-lawn.webp'),
    ('Cleaned and edged landscape bed', 'assets/real-block-border-bed.webp'),
    ('Fence-line bed after cleanup', 'assets/real-fence-line-planting.webp'),
    ('Large lot mowed and cleaned up', 'assets/real-striped-lawn-lot.webp'),
]

def seasonal_cleanups_page():
    filename = 'seasonal-cleanups.html'
    service = 'Seasonal Cleanups'
    faqs = SERVICE_FAQS[service]
    included = [
        ('Leaf removal', 'Leaves blown out of the lawn, beds, fence lines, and from under shrubs, then collected and hauled away.'),
        ('Bed cleanouts', 'Spent perennials cut back, annuals pulled, and leaves and debris cleared out of every bed.'),
        ('Shrub and bush trimming', 'Light shaping and overgrowth cleanup, timed so spring bloomers keep next year\'s flower buds.'),
        ('Debris and brush hauling', 'Sticks, fallen branches, storm debris, and junk-filled corners cleared out and taken away.'),
        ('Clean edges and mulch', 'Bed edges cleaned up, plus a mulch refresh when the beds need it before winter.'),
        ('Final mows', 'The last cuts of the season, lowered gradually so the lawn goes into winter less matted.'),
        ('Aeration and overseeding', 'Pair the cleanup with core aeration and overseeding for a thicker lawn next spring.'),
        ('Snow removal, booked together', 'Lock in seasonal snow service at the same time so the property is covered all winter.'),
    ]
    included_html = ''.join(f'<div><b>{b}</b><span>{t}</span></div>' for b, t in included)
    timeline = [
        ('September', 'Plan and book', 'Cleanup schedules fill once the leaves start dropping, so September is the time to get on the calendar. Late summer into early fall is also the window for core aeration and overseeding.'),
        ('Early to mid October', 'First frost', 'The Toledo area usually sees its first frost in early to mid October. Annuals finish, perennials start going dormant, and beds are ready to be cut back.'),
        ('Late October through November', 'Peak leaf drop: the main cleanup', 'Leaves come down fast. This is the main visit: leaves cleared and hauled off, beds cleaned out, shrubs lightly shaped, and final mows lowered a little at a time.'),
        ('Late November to early December', 'Final pass before snow', 'Wooded lots often need one more pass after the last leaves fall. Then the property is ready for winter and snow service.'),
    ]
    timeline_html = ''.join(f'<div><b>{when}</b><span><strong>{what}.</strong> {detail}</span></div>' for when, what, detail in timeline)
    practices = [
        ('Leaves off the lawn before winter', 'A thick layer of leaves blocks light and can leave thin, matted patches and snow mold. Ohio State University Extension recommends removal once leaves pass about two inches.'),
        ('Spring bloomers left alone', 'Lilac, forsythia, and azalea set next spring\'s flower buds by fall. Shearing them now cuts off next year\'s blooms, so they get pruned right after they flower instead.'),
        ('Light shaping, not heavy cutting', 'Hard fall pruning can push tender new growth right before a freeze. Fall trimming stays light; bigger pruning waits for the right season.'),
        ('Diseased plants cleaned out', 'Perennials with disease or insect problems are cut back and removed so those problems do not carry into spring.'),
        ('Winter interest kept on request', 'Ornamental grasses and coneflower seed heads can stay standing for winter color and for the birds, then come down in the spring cleanup.'),
        ('Final mow lowered gradually', 'The last mows step down toward about 2 to 2&frac12; inches a little at a time, never scalped, so the lawn is less likely to mat under snow.'),
    ]
    practices_html = ''.join(f'<div><b>{b}</b><span>{t}</span></div>' for b, t in practices)
    spring = [
    ...
    spring_html = ''.join(f'<li>{item}</li>' for item in spring)
    related = [
        ('fall-cleanup-checklist-northwest-ohio.html', 'Seasonal guide', 'Fall Cleanup Checklist for Northwest Ohio Properties', 'What to clear, cut back, and leave standing before winter.'),
        ('spring-cleanup-northwest-ohio.html', 'Seasonal guide', 'Spring Cleanup Checklist for Northwest Ohio Properties', 'The spring reset that gets beds and lawns ready for the season.'),
        ('spring-cleanup-cost-toledo-ohio.html', 'Cost guide', 'Spring Cleanup Cost in Toledo and Northwest Ohio', 'What changes the price of a seasonal cleanup.'),
        ('snow-removal.html', 'Winter service', 'Seasonal Snow Removal', 'Book winter service now and the property is covered from fall into snow season.'),
    ]
    related_html = ''.join(f'<a class="guide-card" href="{href}"><span>{cat}</span><h3>{title}</h3><p>{blurb}</p><b>Read more &rarr;</b></a>' for href, cat, title, blurb in related)
    body = f"""
<section class="page-hero premium-page-hero">
  <div class="page-hero-image"><img src="assets/real-fall-leaf-crew.webp" alt="Terramorph crew clearing fall leaves with backpack blowers"></div>
  <div class="hero-overlay"></div>
  <div class="container page-hero-content">
    <p class="crumb"><a href="index.html">Home</a> / Fall and Seasonal Cleanups</p>
    <p class="eyebrow light">Fall + seasonal cleanups</p>
    <h1>Fall cleanups that get the whole property ready for winter.</h1>
    <p>Leaves cleared and hauled away, beds cut back the right way, shrubs shaped, and debris gone. Local crews for Toledo, Perrysburg, and Wood and Lucas County, with spring cleanups when the snow melts.</p>
    <div class="cta-row"><a class="btn btn-gold" href="#request-form">Get a Free Cleanup Estimate</a><a class="btn btn-outline-light" href="tel:{TEL}">Call {PHONE}</a></div>
  </div>
</section>
{trust_band()}
{priority_fit_strip(service)}
<section class="section">
  <div class="container section-heading compact"><p class="eyebrow">Before and after</p><h2>From a leaf-buried yard to a clean lawn that is ready for winter.</h2><p>A wooded Northwest Ohio property before and after a Terramorph fall cleanup.</p></div>
  <div class="container ba-pair">
    <figure><span class="ba-chip">Before</span><img src="assets/real-fall-cleanup-before.webp" alt="Backyard lawn buried in fall leaves before a Terramorph cleanup" loading="lazy"></figure>
    <figure><span class="ba-chip ba-after">After</span><img src="assets/real-fall-cleanup-after.webp" alt="The same property after a Terramorph fall cleanup with a clean striped lawn" loading="lazy"></figure>
  </div>
</section>
<section class="section clay-section">
  <div class="container section-heading compact"><p class="eyebrow">What is included</p><h2>Everything a fall cleanup should cover, in one scheduled visit.</h2><p>Every property is different, so the estimate is built around what your lawn, beds, and trees actually need.</p></div>
  <div class="container cleanup-grid">{included_html}</div>
</section>
{inline_jobber_quote_form('Request a Fall Cleanup Estimate', service).strip()}
<section class="section">
  <div class="container local-grid">
    <div>
      <p class="eyebrow">Built for big leaf loads</p>
      <h2>Commercial blowers and full crews for big leaf loads.</h2>
      <p>Northwest Ohio lots with mature oaks and maples drop a lot of leaves in a few weeks. Terramorph brings commercial walk-behind blowers and backpack crews, so big, wooded properties get cleared and cleaned up without dragging on for days.</p>
      <ul class="check-list"><li>Leaves hauled away - no waiting on city curbside pickup</li><li>Beds, fence lines, and under shrubs, not just the open lawn</li><li>The property left clean, edged, and ready for winter</li></ul>
    </div>
    <figure class="cleanup-photo"><img src="assets/real-fall-windstorm-blower.webp" alt="Terramorph commercial walk-behind leaf blower on a freshly cleaned, striped lawn" loading="lazy"></figure>
  </div>
</section>
<section class="section clay-section">
  <div class="container local-grid">
    <div>
      <p class="eyebrow">Northwest Ohio fall timeline</p>
      <h2>When each part of a fall cleanup should happen around Toledo.</h2>
      <p>Timing matters. Clean up too early and the trees drop another layer; wait too long and wet leaves freeze to the lawn under the first snow.</p>
      <p class="depth-links"><b>City leaf pickup has deadlines.</b> Perrysburg's curbside leaf program ran October 20 to December 1 last year, and piles set out after the final pass were not collected. Terramorph hauls leaves away, so you are not racing a city schedule.</p>
    </div>
    <div class="process-timeline large season-timeline">{timeline_html}</div>
  </div>
</section>
<section class="section">
  <div class="container section-heading compact"><p class="eyebrow">Done the right way</p><h2>What a good fall cleanup leaves alone, and what it takes out.</h2><p>A fast cleanup can still cost you next year's flowers or a healthy lawn. These are the rules Terramorph crews follow on Northwest Ohio properties.</p></div>
  <div class="container cleanup-grid cleanup-grid-3">{practices_html}</div>
</section>
<section class="section clay-section">
  <div class="container local-grid">
    <figure class="cleanup-photo"><img src="assets/real-fall-finished-lawn.webp" alt="Clean, striped lawn under mature trees after a Terramorph seasonal cleanup" loading="lazy"></figure>
    <div>
      <p class="eyebrow">Spring cleanups</p>
      <h2>When the snow melts, the spring cleanup picks up where fall left off.</h2>
      <p>Spring is the reset: whatever winter left behind comes out, and the plants that should be pruned early in the year get handled at the right time.</p>
      <ul class="check-list">{spring_html}</ul>
      <div class="cta-row"><a class="btn btn-primary" href="#request-form">Book a Seasonal Cleanup</a></div>
    </div>
  </div>
</section>
<section class="section work-section"><div class="container section-heading compact"><p class="eyebrow">Cleanup photos</p><h2>Real Terramorph cleanups across Wood and Lucas County.</h2></div><div class="container">{photo_gallery(CLEANUP_PHOTOS)}</div></section>
{faq_section(faqs)}
<section class="section guide-index-section"><div class="container section-heading compact"><p class="eyebrow">Keep reading</p><h2>Seasonal guides for Northwest Ohio properties.</h2></div><div class="container guide-grid">{related_html}</div></section>
<section class="section quote-section"><div class="container quote-grid">{quote_form('Request a Seasonal Cleanup Quote')}<div class="cta-proof"><p class="eyebrow">Why homeowners reach out</p><h2>Crews that show up, clean up, and haul it all away.</h2>{review_stack(3, service)}</div></div></section>
"""
    title = 'Fall and Seasonal Cleanups in Wood and Lucas County | Terramorph'
    desc = 'Fall cleanups, leaf removal, bed cleanouts, and spring cleanups for Toledo, Perrysburg, and Wood and Lucas County. Leaves hauled away. Free estimates.'
    schema = schema_for(filename, 'Fall and Seasonal Cleanups | Terramorph', desc, faqs, service)
    (root / filename).write_text(page(title, desc, body, schema))

seasonal_cleanups_page()
service_page('snow-removal.html', 'Snow Removal', 'Winter snow and ice service', 'snow-removal-truck.webp?v=3.17', 'Lock in dependable snow removal before the first storm.', 'Seasonal snow removal and ice management for driveways, walkways, and commercial lots in Wood and Lucas County. Winter routes fill in the fall - reserve a spot before the first push.', 'Every storm turns into shoveling before work, a blocked driveway, or a commercial lot that is not safe or open when customers arrive.', 'A property that is cleared, treated, and accessible after every qualifying snowfall, on a contract that runs all winter without another phone call.', 'Reserve Winter Service', True)

meta_landing_page('lp-patios.html', 'Paver Patio Estimate', 'Patio and outdoor living estimate', 'real-patio.webp?v=3.14', 'Paver patios and walkways built for Northwest Ohio.', 'Request a free estimate for paver patios, walkways, seating areas, fire pits, and backyard hardscape upgrades.', ['You want a better place to grill, host, relax, or connect the backyard to the home — but need a real plan and estimate first.', 'Terramorph reviews layout, base conditions, drainage, materials, access, and the finished look before scoping the project.', 'A better patio starts with a better plan.', 'The right patio plan connects daily use, backyard flow, drainage, materials, and budget before work starts.'], 'Paver Patios and Hardscapes', 'Request My Patio Estimate', 'Free Paver Patio + Walkway Estimate', ['No useful place to grill or host', 'Old concrete or uneven walkways', 'Backyard feels disconnected from the house', 'Need drainage considered before hardscape work'], [('Layout', 'How the patio connects to doors, grill zones, walkways, seating, and yard flow.'), ('Base + drainage', 'Soil, grade, water movement, and freeze-thaw risk before a patio is priced.'), ('Scope', 'Materials, access, timeline, and budget range for the finished outdoor living space.')], 'Request My Free Patio Estimate')
...
meta_landing_page('lp-aeration-overseeding.html', 'Fall Aeration And Overseeding Estimate', 'Fall core aeration + overseeding', 'real-striped-lawn-front.webp', 'A thicker lawn starts this fall.', 'Request a free estimate for fall core aeration and overseeding. Early fall is the window that gives new grass time to root before winter.', ['Thin, patchy, or compacted lawns in Northwest Ohio rarely fix themselves, and spring seeding fights heat and weeds.', 'Core aeration pulls plugs from the soil so air, water, and nutrients reach the roots, then overseeding fills thin areas with new grass.', 'Timing matters more than anything.', 'Cool nights, warm soil, and fall rain make late August through October the best seeding window of the year.'], 'Fall Aeration & Overseeding', 'Request My Aeration Estimate', 'Free Fall Aeration & Overseeding Estimate', ['Thin or bare spots that never fill in', 'Hard, compacted soil after summer traffic', 'Water that runs off instead of soaking in', 'A lawn that looked tired all summer'], [('Assess', 'We look at soil, thin areas, shade, and how the lawn has been cut and watered.'), ('Aerate', 'Core aeration across the lawn, plugs left to break down and feed the soil.'), ('Seed', 'Quality seed matched to your sun and shade, spread right after aeration so it lands in the holes.')], cta_label='Get My Free Estimate')
meta_landing_page('lp-fall-cleanup.html', 'Fall Cleanup And Leaf Removal Estimate', 'Fall + seasonal cleanups', 'real-fall-leaf-crew.webp', 'Leave the leaves to us.', 'Request a free estimate for fall cleanup: leaf removal, landscape bed cleanouts, mulch refreshes, and yard debris hauled away. Local crews, one visit, done before winter.', ['Leaves, dead perennials, and summer overgrowth pile up fast in Northwest Ohio, and what sits on the lawn through winter smothers the grass underneath.', 'Terramorph crews blow, rake, and haul the leaves, cut back the beds, edge and refresh mulch, and leave the property clean and ready for the season.', 'One crew, one visit, no weekends lost.', 'Wood and Lucas County homeowners get the whole property handled in a single scheduled stop.'], 'Fall & Seasonal Cleanups', 'Request My Cleanup Estimate', 'Free Fall Cleanup Estimate', ['Leaves covering the lawn or beds', 'Overgrown or dead plants in the landscape beds', 'Mulch faded, thin, or full of debris', 'Sticks, brush, and yard debris that need hauling'], [('Walk', 'We look at the lawn, beds, trees, and what needs to be removed or refreshed.'), ('Clear', 'Leaves blown and hauled, beds cut back and cleaned out, debris gone.'), ('Finish', 'Fresh mulch and clean edges so the property looks sharp until spring.')], cta_label='Get My Free Estimate')
meta_landing_page('lp-fall-decor.html', 'Fall Porch And Curb Appeal Decorating Estimate', 'Custom fall decorating', 'real-estate-front-bed.webp', 'Fall curb appeal. Done for you.', 'Request a free quote for custom fall decorating: mums, pumpkins, cornstalks, hay bales, wreaths and lanterns delivered, arranged, and ready to enjoy at your front door or business entrance.', ['A great fall porch takes a trip to the nursery, a truck, and a free Saturday most homeowners and business owners do not have.', 'Terramorph designs the arrangement for your entry, delivers everything, sets it up, and can swap it out for the holidays.', 'Delivered. Decorated. Ready to enjoy.', 'Homes, storefronts, and offices across Wood and Lucas County.'], 'Fall Decorating', 'Request My Fall Decor Quote', 'Free Fall Decorating Quote', ['A front porch or entry that needs a seasonal refresh', 'A storefront or office entrance that should look welcoming', 'No time to shop, haul, and arrange it yourself', 'Want it swapped for winter or holiday decor later'], [('Design', 'We size the arrangement to your steps, columns, and entry.'), ('Deliver', 'Mums, pumpkins, cornstalks, hay, wreaths, and lanterns brought to you.'), ('Decorate', 'Arranged and set in place. You just enjoy it.')], cta_label='Get My Free Quote')
meta_landing_page('lp-landscape-design.html', 'Landscape Design And Installation Estimate', 'Landscape design and installation estimate', 'real-entry.webp?v=3.14', 'Landscape design and installation for Northwest Ohio.', 'Request a free estimate for landscape beds, plantings, mulch, rock, edging, sod, grading-aware installations, and full-yard transformations.', ['The property looks builder-grade, tired, disconnected, or hard to maintain — and you want a clear plan before spending money.', 'Terramorph reviews curb appeal, beds, plantings, grading, access, drainage, maintenance, and how the whole property should feel finished.', 'Design and installation should work together.', 'The right plan connects plants, beds, edges, mulch, lighting, patios, drainage, and upkeep instead of treating them as separate guesses.'], 'Landscape Design', 'Request My Landscape Installation Estimate', 'Free Landscape Design + Installation Estimate', ['Front beds look empty, overgrown, or outdated', 'Not sure what plants/materials fit the house', 'Need mulch, rock, edging, sod, or plant installation', 'Want design, install, and maintenance to connect'], [('Curb appeal', 'What people see from the street, entry, driveway, and outdoor living areas.'), ('Conditions', 'Sun, soil, grade, water movement, maintenance, and existing plant material.'), ('Plan', 'A practical design/install scope that connects budget to the finished look.')], 'Request My Free Landscape Estimate')
...
def city_service_schema(item):
    ...
    desc = f"Terramorph provides {item['keyword']} for homeowners and property owners, with free estimates and direct phone access."
...
<section class="page-hero premium-page-hero">
  ...
  <div class="container page-hero-content"><p class="crumb"><a href="index.html">Home</a> / Local Expertise</p><p class="eyebrow light">Northwest Ohio authority</p><h1>Local expertise for Perrysburg, Toledo, Wood County, and Lucas County.</h1><p>Terramorph serves residential, commercial, and industrial properties with practical experience in drainage diagnosis, hardscape durability, plant health, snow response, and Northwest Ohio soil and weather conditions.</p><div class="cta-row"><a class="btn btn-gold" href="contact.html">Check My Property</a><a class="btn btn-outline-light" href="tel:{TEL}">Call {PHONE}</a></div></div>
</section>
{trust_band()}
{local_authority()}
...
<section class="page-hero premium-page-hero compact-hero quote-request-hero">
  ...
  <div class="container page-hero-content">
    ...
    <p>Submit the secure request form so the Terramorph team receives your property details, photos, service need, and timeline for follow-up.</p>
  ...
  <div class="container quote-request-layout">
    ...
    <div class="quote-request-copy">
      ...
      <div class="authority-list">
        ...
        <div><b>4. Add photos</b><span>Photos and project notes help the team understand visible conditions before follow-up.</span></div>
      </div>
    </div>
  </div>
</section>
...
<section class="section legal-page">
  <div class="container narrow-copy">
    ...
    <p>Terramorph uses Jobber to process quote requests and manage scheduling, customer communication, project records, and follow-up. Information submitted through the embedded or linked Jobber form is also handled under Jobber's privacy practices.</p>
    <h2>Website analytics and advertising</h2>
    <p>This website uses Google Analytics, Google Ads, and the Meta Pixel to understand website traffic, measure marketing performance, and improve advertising. These tools may use cookies or similar identifiers and collect browser, device, page visit, referral, campaign, and interaction information.</p>
...
def write_static_seo_files():
    pages = ['index.html','quote.html','landscape-design.html','paver-patios-hardscapes.html','drainage-solutions.html','outdoor-lighting.html','lawn-maintenance.html','seasonal-cleanups.html','snow-removal.html','guides.html'] + [g['file'] for g in GUIDES] + [p['file'] for p in CITY_SERVICE_PAGES] + ['projects.html','about.html','service-areas.html','contact.html','privacy.html','terms.html']
...
def post_process_html():
    for path in root.glob('*.html'):
        ...
        html = re.sub(r'<script src="app\.js(?:\?v=[^"]+)?"></script>', '<script src="app.js?v=3.63"></script>', html)
