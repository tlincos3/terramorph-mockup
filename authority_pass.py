...
def local_specifics(city: str, service_key: str, faq_html: str) -> str:
    c = CITY_CONTEXT[city]
    return f'''<section class="section local-specifics-section"><div class="container section-heading compact"><p class="eyebrow">Local details</p><h2>{city} property conditions Terramorph plans around.</h2><p>A useful estimate accounts for local soil, drainage, access, neighborhood expectations, and how the property is actually used.</p></div><div class="container local-specifics-grid"><div><b>Common {city} issues</b><span>{c['issues']}.</span></div><div><b>Nearby areas and roads</b><span>{c['places']}.</span></div><div><b>Most relevant services</b><span>{c['services']}.</span></div></div><div class="container"><p class="internal-link-row"><b>Plan the scope:</b> {SERVICE_LINKS.get(service_key, SERVICE_LINKS['landscape'])}</p></div></section>{faq_html}'''


def proof_upgrade_section(review_stack) -> str:
    return f'''<section class="section proof-upgrade-section"><div class="container local-grid"><div><p class="eyebrow">Why homeowners trust Terramorph</p><h2>Local outdoor work backed by reviews, project photos, and clear next steps.</h2><p>Customer feedback, BBB accreditation, completed-work photos, and Northwest Ohio experience give homeowners useful context before requesting an estimate.</p><div class="authority-list"><div><b>Project examples</b><span>Completed-work photos, service pages, local planning guides, and project-specific explanations.</span></div><div><b>Clear process</b><span>Property review, scope diagnosis, material and process explanation, estimate path, and follow-up expectations.</span></div><div><b>Full-service capability</b><span>Landscape design, drainage, hardscape, lighting, maintenance, cleanups, commercial work, snow response, and connected property services.</span></div></div></div><div>{review_stack(3)}</div></div></section>'''
